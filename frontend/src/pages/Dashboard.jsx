...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Total Agents',  value: bots.length,                               icon: Activity,     color: 'text-indigo-400', bg: 'bg-indigo-500/10' },
    { label: 'PAD Agents',    value: pad.length,                                 icon: Bot,          color: 'text-blue-400',   bg: 'bg-blue-500/10' },
    { label: 'Playwright Agents',  value: pw.length,                                  icon: Globe,        color: 'text-emerald-400',bg: 'bg-emerald-500/10' },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {stats.map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} className="bg-white border border-slate-300 shadow-md rounded-xl p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-300 shadow-sm rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-400 flex items-center justify-between">
            <h2 className="font-semibold text-slate-900 text-sm">All Agents</h2>
            <span className="text-xs text-slate-900">{bots.length} total</span>
          </div>
          <div className="divide-y divide-slate-400/50">
            {bots.slice(0, 8).map(bot => (
              <div key={bot.id} className="flex items-center gap-3 px-5 py-3 hover:bg-slate-500/30 transition-colors">
                ...
                <span className="text-black text-sm flex-1 truncate">{bot.name}</span>
            ...
            {bots.length === 0 && <div className="py-10 text-center text-slate-600 text-sm">No Agents yet</div>}
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-300  shadow-sm rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
            <h2 className="font-semibold text-black text-sm">Recent Activity</h2>
            <span className="text-xs text-slate-900">{logs.length} entries</span>
          </div>
          <div className="divide-y divide-slate-600/50">
            {logs.map(log => (
              <div key={log.id} className="flex items-center gap-3 px-5 py-3 hover:bg-slate-800/30 transition-colors">
                ...
                <span className="text-slate-900 text-sm flex-1 truncate">{log.bot_name}</span>
